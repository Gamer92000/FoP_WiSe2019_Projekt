...
In der Methode \textit{calculatePoints(FeatureType type, State state)} werden alle Nodes und Edges im Graph der Reihe nach bearbeitet. Die Reihenfolge ist dabei abh\"angig von den zusammenh\"angenden Strukturen im Spiel, was sich jedoch nicht weiter bedeutend auf die Laufzeit auswirkt.\\
Zu jeder Node werden alle Edges auf Zusammenh\"angigkeit untersucht. Vereinfacht kann man die Komplexit\"at also mit der der folgenden Funktion beschreiben:
\begin{lstlisting}
for (node : graph)
  for (edge : graph)
    [...]
...
And that is complete bullshit. Thank you for coming to my TED Talk. \qed
